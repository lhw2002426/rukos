...
#[no_mangle]
pub unsafe extern "C" fn parse_elf_dyn_glibc(
    ...
    unsafe {
        for i in 0..argc {
            // info!("arg {}", vec_u8_to_str(&ptr2vec(*args.add(i as usize))));
        }
    }
    ...
    let mut at_base = 0;
    let s = b"/lib/libc.so.6\0";
    if prog.interp_path as usize != 0 {
        let interp_prog = super::load_elf::ElfProg::new(prog.interp_path);
        let glibc_prog = super::load_elf::ElfProg::new(s as *const _ as *const c_char);
        entry = interp_prog.entry;
        at_base = interp_prog.base;
        // at_base = glibc_prog.base;
        axlog::ax_println!("AT base is {:x}", at_base);
    };
    ...
    // RUX_ENVIRON has ended with NULL, need not to push NULL
