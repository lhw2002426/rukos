...
impl<'a> SocketSetWrapper<'a> {
    ...
    pub fn poll_interfaces(&self, is_local: bool) {
...
/// packets to the NIC.
pub fn poll_interfaces(is_local: bool) {
    SOCKET_SET.poll_interfaces(is_local);
}
...
pub(crate) fn init(net_dev: AxNetDevice) {
    default_dev.init_by(String::from(net_dev.device_name()));

    let mut dev_wrapper = DeviceWrapper::new();
    let ether_addr = EthernetAddress(net_dev.mac_address().0);
    dev_wrapper.inner.lock().add_device(net_dev);
    //let eth0 = InterfaceWrapper::new("eth0", net_dev, ether_addr);
    let lo_addr = EthernetAddress::default();
    info!("lo addr {}",lo_addr);
    ...
    dev_wrapper.inner.lock().add_device(Box::new(lo_device));
    // lhw TODO: is using ether_addr right?
