...
impl DeviceWrapper {
    pub fn bench_transmit_bandwidth(&mut self) {
        ...
        let mut past_time = InterfaceWrapper::<DeviceWrapper>::current_time();

        // Send bytes
        while send_bytes < MAX_SEND_BYTES {
            if let Some(tx_token) = self.transmit(InterfaceWrapper::<DeviceWrapper>::current_time()) {
            ...
            let current_time = InterfaceWrapper::<DeviceWrapper>::current_time();
    ...
    pub fn bench_receive_bandwidth(&mut self) {
        ...
        while receive_bytes < MAX_RECEIVE_BYTES {
            if let Some(rx_token) = self.receive(InterfaceWrapper::<DeviceWrapper>::current_time()) {
